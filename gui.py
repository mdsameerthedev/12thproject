...
class DataBase:
    ...
    def addAmt(self,accno,amount):
        try:
            query = 'select Account_Number,balance from Account_Holders where Account_Number = %s set balance = balance+%s'
            self.cursor.execute(query,(accno,amount))
            return True
        except:
            return False
    ...
    def transferAmt(self,gaccno,raccno,amount):
        try:
            query = 'select Account_Number,balance from Account_Holders where Account_Number = %s set balance = balance-%s'
            self.cursor.execute(query,(gaccno,amount))
            query2 = 'select Account_Number,balance from Account_Holders where Account_Number = %s set balance = balance+%s'
            self.cursor.execute(query2,(raccno,amount))
            return True
        except:
            return False
...
class display:
    def __init__(self):
        ...
        self.width = res [24:28]
        self.height = res [37:41]
        self.val = self.width + 'x' + self.height
...
class transaction:
    def __init__(self):
        ...
        self.transLabel = ctk.CTkLabel(master=self.transFrame,text='New Transaction',font=('Roboto',30),
                                       width=500,
                                       height=35,anchor='nw')
        ...
        self.transname = ctk.CTkEntry(master=self.transchildFrame,placeholder_text='Enter Your Account Number ',
                                      height=50,
                                      width=450)
        self.transname.pack(pady=20)

        self.rname = ctk.CTkEntry(master=self.transchildFrame,placeholder_text="Receiver's Account Number ",
                                  height=50,
                                  width=450)
        ...
        self.sendButton = ctk.CTkButton(master=self.transchildFrame,text='Transfer',height=50,width=200,
                                        font=('Helvetica',15),command=self.transaction)
        self.sendButton.pack(pady=20,padx=20)

    def transaction(self):
        gaccno = self.transname.get()
        raccno = self.rname.get()
        amount = self.amount.get()
        db.transferAmt(gaccno,raccno,amount)
...
class mainwindow:
    def __init__(self,win_name,details):
        ...
        self.main_lab = ctk.CTkLabel(master=self._mainFrame,text='Home',font=('Roboto',50),height=10,
                                     width=2000,
                                     anchor='w')
        ...
        self.deposit2 = ctk.CTkFrame(master=self.frame1,height=745,width=900)
        self.deposit2.grid(row=1,column=1,pady=5,sticky='nsew')

        self.deposit3 = ctk.CTkFrame(master=self.frame1,height=340)
        self.deposit3.grid(row=2,column=1,sticky='nsew')

        self.AcccBalFrame = ctk.CTkFrame(master=self.frame1,height=340)
        self.AcccBalFrame.grid(row=2,column=0,padx=5,sticky='nsew')
        size = 20
        self._label = ctk.CTkLabel(master=self._accountFrame,text='Account',font=('Roboto',30),height=30,
                                   width=500,
                                   anchor='w')
        self._label.pack(padx=20,pady=20)
        self.Name = ctk.CTkLabel(master=self._accountFrame,text=f'Name: {details [0] [2]}',anchor='w',
                                 height=30,
                                 font=('Roboto',size),
                                 width=400)
        self.Name.pack(pady=10)
        self.acc_No = ctk.CTkLabel(master=self._accountFrame,text=f'Account Number : {details [0] [1]} ',
                                   anchor='w',
                                   font=('Roboto',size),
                                   height=30,width=400)
        ...
        self.type = ctk.CTkLabel(master=self._accountFrame,text=f'Account Type : {details [0] [5]}',
                                 anchor='w',
                                 font=('Roboto',size),
                                 height=30,width=400)
        ...
        self.logout = ctk.CTkButton(master=self._accountFrame,text='Logout',text_color=('red','red'),
                                    font=('roboto',20),height=50,width=100,command=self.logout)
        self.logout.pack(pady=15)
        # --------_______________----------_________-------___________------__________------______----
        self.labelnew = ctk.CTkLabel(master=self.deposit2,text='Payment Options',font=('Roboto',24))
        self.labelnew.pack(pady=15)
        self.transaction = ctk.CTkButton(master=self.deposit2,text='Transfer',height=50,width=400,
                                         command=transaction)
        self.transaction.pack(pady=10,padx=322)

        self.wd = ctk.CTkButton(master=self.deposit2,text='Withdraw',height=50,width=400,command=self.withdraw)
        self.wd.pack(pady=10,padx=322)

        self.transaction = ctk.CTkButton(master=self.deposit2,text='Deposit',height=50,width=400,
                                         command=deposit)
        self.transaction.pack(pady=10,padx=322)

        self.ballabel = ctk.CTkLabel(master=self.deposit3,text='Account Balance: ',width=1040,anchor='nw',
                                     font=('roboto',18))
        self.ballabel.pack(padx=10,pady=10)

        self.balance = ctk.CTkLabel(master=self.deposit3,text=details [0] [7],width=1040,anchor='center',
                                    font=('roboto',100))
        self.balance.pack(padx=10,pady=10)
